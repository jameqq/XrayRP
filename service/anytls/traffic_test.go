...
func TestClearRules(t *testing.T) {
	...
	s := &AnyTLSService{apiClient: emptyRulesAPI{}, config: &controller.Config{UpdatePeriodic: 1}, startAt: time.Now().Add(-time.Hour), logger: log.NewEntry(log.New()), rules: rules, tag: "node"}
